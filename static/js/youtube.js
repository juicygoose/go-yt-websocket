...
socket.onmessage = function (e) {
  ...
  if (obj.nextVideos) {
    document.getElementById("output").innerHTML +=
      "here is next videos data" + e.data + "\n";
  ...
  if (obj.playVideoId) {
    document.getElementById("output").innerHTML +=
      "This is the json with video id: " + obj.playVideoId + " \n";
    if (obj.playVideoId == youtubeVideoId) {
      document.getElementById("output").innerHTML +=
        "Nothing to do as the video playing is the same than video received\n";
    } else {
      if (e.data.includes("playTime")) {
        document.getElementById("output").innerHTML +=
          "Going to play at a specific time\n";
  ...
  if (obj.cueVideoId) {
    if (obj.remove == false) {
      document.getElementById("output").innerHTML +=
        "Cue req with id: " + obj.cueVideoId + " \n";
      nextVideos.push(obj);
...
function addChatText(name, message) {
  ...
  document
    ...
    .scrollTo(0, document.getElementById("chatroom").scrollHeight);
}
