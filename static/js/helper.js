    ...
    if (request.status >= 200 && request.status < 400) {
      for (var i = 0; i < data.length; i++) {
        document.getElementById("rooms-dropdown").innerHTML =
          formatNavbarItem(data[i].Name, data[i].NumberOfClients) +
          document.getElementById("rooms-dropdown").innerHTML;
      }
    } else {
...
function activatepanel(panelToActivate) {
  ...
  if (master || panelToActivate == "search") {
    ...
    document
      .getElementById("track-toolbox-" + activeRecordShelfPanel)
      .setAttribute("style", "display: none;");
    document
      .getElementById("track-toolbox-" + panelToActivate)
      .removeAttribute("style");
    document
      .getElementById("panel-tab-" + activeRecordShelfPanel)
      .classList.remove("is-active");
    document
      .getElementById("panel-tab-" + panelToActivate)
...
function activateSocialPanel(panelToActivate) {
  ...
  document
    .getElementById("social-panel-" + activeSocialPanel)
    .setAttribute("style", "display: none;");
  document
    .getElementById("social-panel-" + panelToActivate)
    .removeAttribute("style");
  document
    .getElementById("panel-tab-" + activeSocialPanel)
    .classList.remove("is-active");
  document
    .getElementById("panel-tab-" + panelToActivate)
    .classList.add("is-active");
